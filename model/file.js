 ...
 * @param {Function} cb 回调函数
 */
exports.creat = function(dirName,cb){
 ...
 * @param {Function} cb  回调函数
 */
exports.updata = function(dirName,pic,cb){
    // 获取文件的旧路径
    ...
    // 设置新的文件名 
    // var n = new DataCue().getTime();
    var str = sd.format(new Date(),"YYYYMMDDHHmmss")+ext;
                                            //20201023185546.jpg
    // 改名(修改路径)
    // 新的路径
    var newPath = './uploads/'+dirName+'/'+str;
    fs.rename(oldPath,newPath,function(err){// 上传方法
      cb(err);
    });
    console.log(str);
}
